...
AXL_TODO("join DoxyParser and DoxyMgr")

DoxyGroup g_group;
DoxyFootnote g_footnote;
...
/*		if (entry.m_namespaceId == m_module->m_namespaceMgr.getCurrentNamespace())
		{
			if (!doxyBlock)
...
//				DoxyGroup* group = m_module->m_doxyMgr.getGroup(nextToken->m_data.m_string);
		...
		case DoxyTokenKind_OpeningBrace:
			if (m_block->getBlockKind() == DoxyBlockKind_Group)
			{
				...
				entry.m_namespaceId = 0;
//				entry.m_namespaceId = m_module->m_namespaceMgr.getCurrentNamespace();
